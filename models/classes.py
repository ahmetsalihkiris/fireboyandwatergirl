...
class Ates1(pygame.sprite.Sprite):
    ...
    def update(self, engeller, merdivenler, lav_havuzlari, su_havuzlari, zehir_havuzlari):
        self.hiz_x = 0
...
class Su1(pygame.sprite.Sprite):
    ...
    def update(self, engeller, merdivenler, su_havuzlari, lav_havuzlari, zehir_havuzlari):
        self.hiz_x = 0
        tuslar = pygame.key.get_pressed()
...
class CSVHarita:
    def __init__(self):
        ...
        self.katmani_yukle("map/harita_1/harita_1_sarmasik.csv", self.merdivenler)
            ...
            with open(dosya_yolu, mode='r', encoding='utf-8') as dosya:
                ...
                for y_indeks, satir in enumerate(okuyucu):
                    for x_indeks, tile_id in enumerate(satir):
                        val = tile_id.strip()
                        if val != "" and int(val) > -1: # Sadece -1'den büyükse (boş değilse) işle
                            _id = int(val)
